...
export const POST: APIRoute = async ({ request, params, redirect }) => {
    ...
    const data: {action: string, content: string, idTablero: string} = await request.json();
    ...
    if(accion == "borrar-completas")
    {
        ...
        console.log("desde el backend : ", tasks)
        return new Response(JSON.stringify({ tasks }), { status: 201 });

    }else if(accion == "agregar-tarea"){
        const tarea = addTarea(data.content, data.idTablero);
